...
# Salvo le medie dell'Overall, Potential e faccio anche la differenza tra Potential e Overall effettivo (per vedere lo sviluppo del giocatore o quanto potrebbe diventare forte) gruppati per Nazionalità. Ci servirà per generare i grafici successivi.
...
# Mergiamo i dati del dataset world con la variabile overall_data che abbiamo creato prima, perchè contiene il Count dei giocatori per Nazionalità. Questa nuova variabile ci servirà per creare i grafici successivi:

merged_data <- merge(x = worldmap, y = overall_data, by.x = "region", by.y = "Nationality", all.x = TRUE) %>% arrange(order)

# Grafico conteggio del numero di giocatori per Nazionalità. Lo applichiamo a merged_data perchè lì c'è anche il Count dei giocatori per Nazionalità:
...
# Grafici Average Overall, Potential e Differenza tra Potential e  Overall. Per generarli utilizziamo merged_data che abbiamo creato prima: 
